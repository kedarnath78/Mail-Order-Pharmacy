...
class RefillControllerTest {
	...
	void viewRefillStatusTestByMemberId() throws Exception {
		list.add(new RefillOrder(1, LocalDate.now(), true, (long) 1, 10, "priya"));
		list.add(new RefillOrder(2, LocalDate.now(), true, (long) 1, 5, "priya"));
		when(repos.findAll()).thenReturn(list);

		finallist = list.stream().filter(p -> p.getMemberId().equals("priya")).collect(Collectors.toList());
		...
		when(refillOrderService.getStatusByMember("priya", "Bearer Token")).thenReturn(finallist);
		MvcResult result = mockMvc
				.perform(get("/api/viewRefillStatusforAll/{mid}", "priya").header("Authorization", "Bearer Token"))
				.andReturn();
	...
	void deleteBySubscriptionId() throws SubscriptionIdNotFoundException, InvalidTokenException {
		...
		ArrayList<RefillOrder> list = new ArrayList<>();
		RefillOrder refillOrder = new RefillOrder(1, LocalDate.now(), true, 1, 3, "priya");
		RefillOrder refillOrder2 = new RefillOrder(1, LocalDate.now(), true, 2, 3, "priya");
		list.add(refillOrder);
		ValidateToken validateToken = new ValidateToken("priya", "priya", true);
		ResponseEntity<ValidateToken> response = new ResponseEntity<ValidateToken>(validateToken, HttpStatus.OK);
		when(authClient.getValidity("token")).thenReturn(response);
